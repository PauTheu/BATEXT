...
In Kapitel \ref{ch:jaffe} haben wir den Technologieraum für die Firma Honda und ihre Konkurrenz nach dem Ansatz von \parencite{jaffe1989characterizing} dargestellt und interpretiert. Grundlage dabei war es die Firmen aufgrund von Patentklassen zu unterscheiden. Als nächstes wollen wir versuchen etwas tiefer in die Daten zu schauen und dabei die gemeinsamen Wissensgrundlagen der Firmen zu untersuchen. In diesem Kapitel wird ein Technologieraum auf Basis von Patentzitaten vorgestellt. Zunächst werde ich die Paper \textcite{stuart1996local} und \textcite{kitahara2017technology} zusammenfassen und anschließend die darin vorgestellten Modelle implementieren.
...
\subsection{Der Technologieraum für die Firma Honda und ihre Konkurrenz nach \textcite{stuart1996local}}
...
def getAlpha(i,j):
    ...
    # Anteil der Patente, die Firma j zitiert, die auch von Firma i zitiert werden.
    aij = len(patSchnitt) / len(patJ)
    return aij
\end{lstlisting}
