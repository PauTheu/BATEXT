...
In diesem Kapitel wird ein Technologieraum auf Basis von Patentzitaten vorgestellt. Zunächst werde ich die Paper \parencite{stuart1996local} und \parencite{kitahara2017technology} zusammenfassen und anschließend die darin vorgestellten Modelle implementieren.
...
Die $\alpha$'s sind die Wettbewerbskoeffizienten der i'ten und j'ten Dyaden zum Zeitpunkt $t_m$. Der erste Term $(\alpha_{kit_m} - \alpha_{kjt_m})$ beschreibt die Differenz der Nischenüberschneidungen zwischen Firma i und anderen Firmen k und der Nischenüberschneidungen von Firma j und anderen Firmen k. Der zweite Term $(\alpha_{ikt_m} - \alpha_{kjt_m})$ ist die Gegenrichtung. Dabei wir die Differenz der technologischen Nischen der Firma k und denen der Firmen i und j berechnet. 

Um die Distanz zwischen Firmen zu zwei verschiedenen Zeitpunkten festzuhalten, wird im nächsten Schritt eine Matrix nach folgender Metrik definiert. 
...
\textcite{stuart1996local} wenden die oben beschrieben Methodik an um Positionsveränderungen zehn japanischen Firmen in der Halbleiterindustrie darzustellen. Für den Technologieraum wurden die Jahre 1982, 1987 und 1992 gewählt. Da es zehn Firmen zu je drei Zeiträumen gibt, hat der resultierende Graph 30 Punkte. Die Autoren teilen den Technologieraum zunächst in zwei Gruppen ein. Auf der einen Seite befinden sich Firmen, die einfache, auf den Mainstream-Konsum basierte Technologien produzieren. Auf der anderen Seite befinden sich Unternehmen die sich in komplexeren, industriellen Technologiesektoren aufhalten. So produzieren diese Firmen die Kerntechnologien die in einem jeden Computer stecken (beispielsweise Schaltungslogik für Prozessoren). 
...
\caption[MDS der japanische Halbleiterindustrie]{Technologische Positionen von Firmen in der japanischen Halbleiterindustrie \parencite[S. 30]{stuart1996local}}
\label{mitsu}
\end{figure}
...
\subsection{Erweiterung des Ansatzes}	
...
\subsection{Implementierung des Modells und Intepretation der Ergebnisse}
\label{abs:mdsstuart}
...
Um ein möglichst Umfangreiches Bild zu schaffen betrachten wir Patentdaten für den Gesamtzeitraum (1970 - 2019). Für unsere zehn Firmen erhalten wir eine 10x10 community matrix. Anschließend berechnen wir die Firmendistanzen nach der oben definierten Metrik \ref{sequ1}. Im letzten Schritt wenden wir wieder die multidimensionale Skalierung auf unsere Distanzmatrix an.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\linewidth]{files/mdsstuart1.PNG}
\caption[MDS Patentzitate]{Graph der multidimensionalen Skalierung nach Patentzitaten}
\label{mdsstuart1}
\end{figure}

Der Graph in Abbildung \ref{mdsstuart1} zeigt den Technologieraum nach \textcite{stuart1996local} für unsere zehn Firmen. Auf den ersten Blick erkennen wir große Unterschiede zu unserem Graphen in Kapitel \ref{ch:jaffe}. So sehen wir Toyota alleine im linken Teil des Technologieraumes. Auf der anderen Seite befinden sich die restlichen neun Unternehmen eng gruppiert. Trotz der offensichtlichen Unterschiede erkennen wir kleinere Gemeinsamkeiten zu unserem Technologieraum in Kapitel \ref{ch:jaffe}. Um diese Gemeinsamkeiten besser aufzuzeigen, betrachten wir in Abbildung \ref{mdsstuart2} den Technologieraum ohne Toyota. Wir können diese Änderung im MDS-Graphen relativ bedenkenlos durchführen, da die Distanzen zwischen Toyota und allen anderen Firmen relativ gleich ausfallen.


\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\linewidth]{files/mdsstuart2.PNG}
\caption[MDS Patentzitate ohne Toyota]{Graph der multidimensionalen Skalierung nach Patentzitaten ohne die Firma Toyota}
\label{mdsstuart2}
\end{figure}

Wir beobachten weiterhin Unterschiede der Firmenkonstellation im Vergleich zu Kapitel \ref{ch:jaffe}. Allerdings erkennen lassens sich gewisse Konstanten in der Gruppierung der Firmen feststellen. Wir erkennen sowohl die Firmengruppierung der Automobilzulieferer Denso und Bosch, als auch die Gruppierung der Konkurrenten Honda und Nissan. Die Firma Mazda befindet sich wieder am Rand des Technologieraums. 

Wir wollen versuchen die Position von Toyota zu begründen. Wie in Kapitel \ref{ch:jaffe} bereits gezeigt wurde nimmt die Firma Toyota eine Sonderstellung ein. Die Firma besitzt in der von uns betrachteten Patentklasse mit Abstand die meisten Patente. Eine große Patentzahl impliziert indirekt eine hohe absolute Zahl an Patentzitationen. Abbildung \ref{anzahlzit} bestätigt diese Annahme. Diese Tatsache führt dazu, dass Toyota eine sehr breite technologische Nische einnimmt. Da die restlichen Unternehmen nur einen geringen Teil dieser technologischen Nische besetzen können, werden die Wettbewerbskoeffizienten ($\alpha_{ij}$) dieser Unternehmen im Vergleich zu Toyota relativ klein ausfallen. Zusätzlich ist die von \textcite{stuart1996local} definierte Metrik, eine euklidische Distanzmetrik. Die Vektorlängen der neun Unternehmen sind sehr klein, die Länge des Vektors der Firma Toyota jedoch relativ groß. Im Ergebnis erhalten wir schließlich die ausfallende Position von Toyota im Technologieraum. 
...
\caption[MDS Patentzitate mit LG]{Graph der multidimensionalen Skalierung nach Patentzitaten mit der Firma LG}
\label{mdsstuart3}
\end{figure}

\subsection{Erweiterung des Modells und Intepretation der Ergebnisse}
...
Abbildung \ref{mdskit1} zeigt den Graph der multidimensionalen Skalierung nach Patentzitaten inklusive Patentzitaten der zweiten Ebene für einen Diskontierungsfaktor von 0.6. Wir beobachten eine Veränderung des Technologieraums im Vergleich zu Abschnitt \ref{abs:mdsstuart}. Das Zulassen der second-order Zitate \glqq glättet\grqq{} das Ergebnis, da es jetzt zu deutlich mehr Überschneidungen in Patentzitaten kommt. Je höher wir $\eta$ wählen desto größer ist der Einfluss der second-order Zitate. Das Verhältnis der Firmen zueinander ändert sich dabei wenig, allerdings rücken die Firmen näher zusammen (die Achsen des MDS-Graphen werden kleiner). 

Für diesen Ansatz fallen die absoluten Distanzen zwischen allen Firmen insgesamt höher aus. Dennoch sind die Ergebnisse relativ konsistent zu den Ergebnissen in Kapitel \ref{ch:jaffe}. Wir beobachten Mazda weit entfernt von anderen Firmen am Rand des Technologieraums. Die Firmen Bosch und Denso stehen sich wiederum nahe. Weiterhin finden wir Toyta, Nissan und Honda im selben Quadranten des Technologieraums. Auch die Positionen der Firmen GM, Ford und Hyundai verhalten sich ähnlich zu deren Positionen in Kapitel \ref{ch:jaffe}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\linewidth]{files/mdskit1.PNG}
\caption[MDS Patentzitate der zweiten Ebene]{Graph der multidimensionalen Skalierung nach Patentzitaten inklusive second-order Zitate $\eta = 0.6$}
\label{mdskit1}
\end{figure}

In \parencite{kitahara2017technology} wird die gesamte Technologieraumformation für einen Wirtschaftszweig betrachtet. Wir wollen versuchen eine größere Anzahl an Firmen zu betrachten um möglicherweise eine Polarisation im Innovationssektor der Automobilbranche festzustellen. Um die Übersichtlichkeit des Graphen zu gewährleisten wählen wir die Top 31\footnote{Top 30 Unternehmen und die Firma Porsche (Platz 31).} Unternehmen (nach Patentzahl) und untersuchen deren Anordnung in unserem Technologieraum (Abbildung \ref{mdskit2}). 

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\linewidth]{files/mdskit2.PNG}
\caption[MDS Patentzitate der zweiten Ebene, 31 Firmen]{Graph der multidimensionalen Skalierung nach Patentzitaten inklusive second-order Zitate für 31 Firmen}
\label{mdskit2}
\end{figure}

Wir erkennen einen Pol um den sich die etablierten OEM's verteilen. An der Peripherie des Pols befinden sich weniger etablierte Automarken und Unternehmen aus der Elektronik- und Halbleiterbranche. Intuitiv ergibt dieses Bild Sinn. Auch wenn nicht-Automobilunternehmen einige Patente der $Y02T\_10$-Kategorie halten, werden sich ihre Wissensgrundlagen weniger mit den Unternehmen der Automobilindustrie decken. Zudem vernetzen sich die Unternehmen der Automobilindustrie gerade bei der Suche nach nicht fossilen Energieträgern in den letzten Jahren kontinuierlich\footnote{Quelle: \href{http://competitionlawblog.kluwercompetitionlaw.com/2019/11/22/automotive-industry-is-moving-towards-electrification-via-joint-ventures/}{competitionlawblog.com} Stand: 26.10.2020.} (Vollständiger Code siehe \ref{appendix:stuart}).
