...
\begin{equation}
\label{sequ2}
d_{it_ljt_m} \equiv d_{jt_mit_l} = \bigg\{ \sum_{k=1}^{n} [(\alpha_{ikt_l} - \alpha_{jkt_m})^2 + (\alpha_{kit_l} - \alpha{_kjt_m})^2]\bigg\}^\frac{1}{2}, k \neq i, j
\end{equation}
...
\caption{Zwei Firmen und ihre jeweiligen Patentzitate}
\label{kitbsp}
\end{figure}
...
\item eine firma hat sich im technology space stark bewegt
